...
Applies the privacy-oriented end-to-end encryption modifications to an ai-toolkit installation:
  - Creates toolkit/crypto.py (AES-256-GCM backend engine)
  - Creates ui/src/utils/crypto.ts (WebCrypto in-browser encryption & EXIF stripping)
  - Updates Settings server & API to generate and persist encryption passwords
  - Updates Job worker to pass the encryption key in RAM and encrypt config files
  - Updates dataset upload, viewing, and caching routines to encrypt/decrypt on-the-fly
  - Sanitizes LoRA safetensors output metadata and masks prompt logs
...
def main():
    ...
    # 11. Patch ui/cron/fileServer.ts to skip server-side thumbnail generation for encrypted files
    fileserver_path = target_dir / "ui" / "cron" / "fileServer.ts"
    if fileserver_path.exists():
        content = fileserver_path.read_text(encoding="utf-8")
        if "AITK_ENC_V1" not in content:
            content = content.replace(
  ...
  try {
    ...
    return false;
            ...
            fileserver_path.write_text(content, encoding="utf-8")
            print(f"  [+] Patched: {fileserver_path.relative_to(target_dir)}")

    print("\nPatch complete! All privacy-oriented encryption modules and recent upstream updates are integrated.")
